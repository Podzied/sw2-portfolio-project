...
I'm using a java.util.List<Song> as my representation. Specifically ArrayList.

Why this works well:
- playlist is basically just a sequence of songs so a list is a natural fit
- ArrayList gives O(1) access by index which is good for getSongAt
- addSong just appends to end which is O(1) amortized for ArrayList
- removeSong at index is O(n) but thats about as good as it gets for a sequence
- length is O(1)

I thought about using a linked list but random access would be O(n) which would hurt getSongAt. Array would work but Id have to handle resizing manually which ArrayList already does.
