...
- **Name**: Ayush Saggar
- **Dot Number**: saggar.9
- **Due Date**: 10/9/2025 @ 11:59 PM EST
...
i picked the music playlist component for my proof-of-concept. here's why:

1. **personal interest**: i actually use spotify playlists all the time, so this feels relevant to my life. if i'm going to spend a semester on something, it should be something i care about.

2. **time constraints**: this is just a proof-of-concept, not the full project. i need something i can actually finish without getting stuck on complex details. a playlist is basically just a list of songs - pretty straightforward.

3. **learning curve**: i'm still figuring out the whole kernel/secondary thing. playlists have obvious operations (add, remove songs) that map clearly to this design pattern. less chance i'll mess up the architecture.

4. **real-world stuff**: everyone uses playlists. it's not some abstract concept that only programmers understand. good for showing employers.

5. **my limitations**: let's be honest - i'm still learning this stuff. the playlist operations are intuitive enough that i won't get lost in the weeds trying to figure out what methods should go where.

6. **portfolio value**: when i show this to people, they'll immediately get it. "oh, like spotify!" way better than trying to explain some weird data structure.

the weather tracker and color conversion ideas were cool too, but the playlist hits the sweet spot of being doable and actually useful.
